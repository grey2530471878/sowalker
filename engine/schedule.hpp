...
public:
    ...
    virtual bid_t schedule(graph_cache &cache, graph_driver &driver, graph_walk &walk_manager,bool part) {
        return 0;
    }
...
public:
    ...
    bid_t schedule(graph_cache &cache, graph_driver &driver, graph_walk &walk_manager,bool part)
    {
...
private:
    ...
    void choose_blocks(graph_cache &cache, graph_driver &driver, graph_walk &walk_manager,bool part)
    {
        ...
        for(auto blk : bucket_uncached) {
            if(partition_walks[blk] >= 0) {
                ...
                if(part==0)
                {
                    ...
                    cache.cache_blocks[pos].block=&walk_manager.global_blocks->blocks[blk];
                    cache.cache_blocks[pos].block->cache_index=pos;
                    cache.cache_blocks[pos].block->status=PART;
                    cache.cache_blocks[pos].beg_pos=NULL;
                    cache.cache_blocks[pos].weights=NULL;
                }
                pos++;
            }
        }
...
public:
    ...
    bid_t schedule(graph_cache &cache, graph_driver &driver, graph_walk &walk_manager,bool part)
    {
        if(part==0)
        {
            ...
            choose_blocks(cache, driver, walk_manager,part);
            _m.stop_time("simulated_annealing_scheduler_swap_blocks");
        }
        else
        {
            _m.start_time("simulated_annealing_scheduler_swap_blocks_part");
            choose_blocks(cache, driver, walk_manager,part);
            _m.stop_time("simulated_annealing_scheduler_swap_blocks_part");
        }
        return 0;
    }
};
