...
public:
    ...
    void run(programtype &userprogram, scheduler *block_scheduler)
    {
        ...
        while(!walk_manager->test_finished_walks()) {
...
#endif  
            if(total_walks>conf->nvertices*1*0.005)//全部加载
            //if(1)
            {
                logstream(LOG_DEBUG) << "全部加载 " << std::endl;
                block_scheduler->schedule(*cache, *driver, *walk_manager,0); 
            size_t pos = 0;
            ...
            while(pos < cache->walk_blocks.size()) {
                ...
                update_walk(userprogram, nwalks,0);
            ...
            logstream(LOG_DEBUG) << "部分加载 " << std::endl;
                block_scheduler->schedule(*cache, *driver, *walk_manager,1); 
            size_t pos = 0;
            logstream(LOG_DEBUG) << "cache walk block size : " << cache->walk_blocks.size() << std::endl;
            std::cout << "cache index : ";
            for(bid_t blk = 0; blk < walk_manager->global_blocks->nblocks; blk++) {
            ...
            while(pos < cache->walk_blocks.size()) {
                ...
                update_walk(userprogram, nwalks,1);
                logstream(LOG_DEBUG) << "load memory walks, pos = " << pos << ", walks = " << nwalks << std::endl;
            }
            pos = 0;
            while (pos < cache->walk_blocks.size()) {
    ...
    template <class programtype>
    void update_walk(programtype &userprogram, wid_t nwalks,bool part)
    {
        ...
        _m.start_time("exec_block_walk");
            ...
            for(wid_t idx = 0; idx < nwalks; idx++) {
                ...
                {
                    //加载部分边
                    bid_t cur_cache_index = (*(walk_manager->global_blocks))[walk_manager->walks[idx].cur_index].cache_index, prev_cache_index = (*(walk_manager->global_blocks))[walk_manager->walks[idx].prev_index].cache_index;
                    _m.start_time("part_func_time_all");
                    driver->load_block_info_part(walk_manager->walks[idx],*cache,cur_cache_index,prev_cache_index);
                    _m.stop_time("part_func_time_all");
                }
                userprogram.update_walk(walk_manager->walks[idx], cache, walk_manager, &seeds[omp_get_thread_num()],part);
            }
#ifdef PROF_STEPS
            total_times++;
